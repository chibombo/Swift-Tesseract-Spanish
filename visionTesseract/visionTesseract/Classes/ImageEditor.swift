...
extension UIImage{
    ...
    func GARFilter() -> UIImage?{
        ...
        if let imageOutPut = filter.outputImage?.applyingFilter("CIColorControls", parameters: [kCIInputContrastKey: 3, kCIInputBrightnessKey: 0.78, kCIInputSaturationKey: 0]), let image = context.createCGImage(imageOutPut, from: imageOutPut.extent){
